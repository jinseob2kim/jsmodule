...
#' @description File upload UI for survey data analysis.
#' @param id id
#' @param label label, Default: 'csv/xlsx/sav/sas7bdat/dta file'
#' @return File upload UI for survey data analysis.
...
#' @description File upload server module for survey data analysis.
#' @param input input
#' @param output output
#' @param session session
#' @param nfactor.limit nfactor limit to include, Default: 20
#' @return File upload server module for survey data analysis.
...
FileSurvey <- function(input, output, session, nfactor.limit = 20) {
  ...
  data <- eventReactive(input$file, {
    ...
    # ref <- data.table::data.table(name.old = name.old, name.new = name.new);setkey(ref, name.new)
  ...
  observeEvent(input$factor_vname, {
    output$survey <- renderUI({
      ...
      tagList(
        ...
        selectInput(session$ns("cluster_vname"),
          label = "Cluster ID",
          choices = c("None", names(data()$data)), multiple = F,
          selected = selected.cluster.final
        ),
        selectInput(session$ns("strata_vname"),
          label = "Strata",
          choices = c("None", names(data()$data)), multiple = F,
          selected = selected.strata.final
        ),
        selectInput(session$ns("weights_vname"),
          label = "Weights",
  ...
  outdata <- reactive({
    ...
    out <- data.table::data.table(data()$data)
    ...
    surveydata <- tryCatch(survey::svydesign(id = cluster.survey, strata = strata.survey, weights = weights.survey, data = out),
      error = function(e) {
        return(survey::svydesign(id = cluster.survey, strata = strata.survey, weights = weights.survey, data = out, nest = T))
      }
    )
