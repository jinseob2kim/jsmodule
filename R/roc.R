...
ROC_table <- function(ListModel, dec.auc =3, dec.p = 3){
  ...
  auc.ci <- sapply(ListModel, function(x){paste0(round(pROC::ci.auc(x)[1], dec.auc), "-", round(pROC::ci.auc(x)[3], dec.auc))})
  if (length(ListModel) == 1){
    ...
    info.diff <- data.table::rbindlist(
      lapply(seq_along(ListModel),
             function(x){
               if (x == 1) return(data.frame(t(rep(NA, 7))))
               ...
               reclass <- reclassificationJS(data = data.frame(out.int), cOutcome = 1,
    ...
    names(out) <- c("Prediction Model", "AUC", "95% CI", "P-value for AUC Difference", "IDI", "95% CI", "P-value for IDI",
