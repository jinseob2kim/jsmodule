    ...
    if (is.null(design.survey)) {
      ...
      if (default.unires) {
        varsIni <- sapply(
          ...
          function(v) {
            ...
            coef <- tryCatch(summary(glm(forms, data = data()))$coefficients, error = function(e) {
              return(NULL)
            })
      ...
      if (default.unires) {
        varsIni <- sapply(
          ...
          function(v) {
            forms <- as.formula(paste(input$dep_vars, "~", v))
            coef <- tryCatch(summary(survey::svyglm(forms, design = design.survey()))$coefficients, error = function(e) {
              return(NULL)
            })
...
logisticModule2 <- function(input, output, session, data, data_label, data_varStruct = NULL, nfactor.limit = 10, design.survey = NULL, default.unires = T, limit.unires = 20, vec.event=NULL) {
  ...
  vlist <- reactive({
    ...
    except_vars <- factor_vars[nclass_factor > nfactor.limit | nclass_factor == 1 | nclass_factor == nrow(data())]
    return(list(
      factor_vars = factor_vars, factor_list = factor_list, nclass_factor = nclass_factor, factor2_vars = factor2_vars,
    ...
    if (is.null(design.survey)) {
      ...
      if (default.unires) {
        varsIni <- sapply(
          ...
          function(v) {
            forms <- as.formula(paste(input$dep_vars, "~", v))
            coef <- tryCatch(summary(glm(forms, data = data(), family = binomial))$coefficients, error = function(e) {
              return(NULL)
            })
      ...
      if (default.unires) {
        varsIni <- sapply(
          ...
          function(v) {
            forms <- as.formula(paste(input$dep_vars, "~", v))
            coef <- tryCatch(summary(survey::svyglm(forms, design = design.survey(), family = binomial))$coefficients, error = function(e) {
              return(NULL)
            })
            sigOK <- ifelse(is.null(coef), F, !all(coef[-1, 4] > 0.05))
  ...
  out <- reactive({
    ...
    if (input$regressUI_subcheck == T) {
      validate(
        ...
        need(all(sapply(1:length(input$subvar_logistic), function(x) {
          length(input[[paste0("subval_logistic", x)]])
        })), "No value for subsetting")
      )
